...
describe('resolve', () => {
  it('throws an error if file not found', async () => {
    let failed = false;
    try {
      await resolve(__dirname + '/fixtures/error-case');
    } catch (e) {
  ...
  it('throws an error if circular reference found', async () => {
    let failed = false;
    try {
      await resolve(__dirname + '/fixtures/circular');
    } catch (e) {
      expect(e.message).toEqual(
  ...
  it('throws an error if invalid sql found', async () => {
    let failed = false;
    try {
      await resolve(__dirname + '/fixtures/invalid');
    } catch (e) {
      expect(e.message).toEqual(
  ...
  it('resolves sql in proper order', async () => {
    let sql = await resolve(__dirname + '/fixtures/basic');
