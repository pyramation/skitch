...
import { listModules, latestChange, getPlan, getExtensionsAndModules } from '../index'

const cleanText = t => t.split('\n').map(a=>a.trim()).filter(a=>a).join('\n');

describe('sqitch modules', () => {
  it('should get modules', async () => {
    const modules = await listModules();
    expect(modules).toEqual({
      "totp": {"path": "packages/totp", "requires": ["plpgsql", "uuid-ossp"], "version": "0.0.1"},
      "utils": {"path": "packages/utils", "requires": ["plpgsql", "uuid-ossp", "totp"], "version": "0.0.1"}
    });
  });
